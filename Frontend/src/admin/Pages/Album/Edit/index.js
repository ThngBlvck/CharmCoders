...
import { postImage, getOneImage, updateImage } from "../../../../services/Album"; // Thêm hàm lấy chi tiết album và cập nhật album
...
export default function EditAlbum({ color = "light" }) {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            await updateImage(id, formData); // Cập nhật album với dữ liệu mới

            Swal.fire('Thành công', 'Cập nhật album thành công.', 'success').then(() => {
                reset();
                setSelectedImages([]);
                setAlbumImages([]);
                setProductId("");
            ...
            Swal.fire('Lỗi', 'Lỗi khi cập nhật album. Vui lòng thử lại.', 'error');
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div className="p-4">
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            {Array.isArray(selectedImages) && selectedImages.length > 0 && (
                                <div className="mb-4">
                                    <label className="block text-gray-700 text-sm font-bold mb-2">Hình ảnh đã
                                        chọn:</label>
                            ...
                            <div className="mb-4">
                                ...
                                {Array.isArray(albumImages) && albumImages.length > 0 && (
                                    <div className="mb-4">
                                        <label className="block text-gray-700 text-sm font-bold mb-2">Album hình ảnh đã
                                            chọn:</label>
                                        <div className="flex flex-wrap gap-4">
